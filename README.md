<p align="center"><img src="https://github.com/user-attachments/assets/3590946b-f4d1-4f4c-bd21-22d4ebe7263a"></p>

<h1 align="center">Hi 👋, I'm Shubhanshu Arya</h1>
<h3 align="center">Portfolio: <a href="https://thisisshubh.com">thisisshubh.com</a> | LinkedIn: <a href="https://www.linkedin.com/in/shubharya">shubharya</a></h3>
<br />

```console
shubhanshu@github:~$ whoami
Shubhanshu Arya

shubhanshu@github:~$ cat about_me.yaml
```
```yaml
name: Shubhanshu Arya
role: Software Development Engineer
location: New Delhi, India

saas_startup:
  name: WellPilot
  description: B2B Platform for Health & Nutrition Clubs

education:
  - Dual Bachelor's Degree in Computer Science

fields_of_interests:
  - Open Source Developer
  - Full Stack Web Development
  - Mobile App Development
  - Problem Solving
  - UI/UX
  - Web3

experience:
  JoinUp:
    role: Software Development Engineer
  VILS AI:
    role: Software Engineering Intern
  Refold AI:
    role: Software Engineering Intern
  Aican Automate:
    role: Machine Learning Intern

coding_profiles:
  leetcode: _shubh_
  geeksforgeeks: _shubh_
  codechef: aryashubhanshu

currently: Figuring out startups
contact_me_at: aryashubhanshu1@gmail.com
```
```console
shubhanshu@github:~$ ./status.sh
⚙️ Turning complex problems into elegant, scalable software solutions.
🤝 Currently exploring new SDE roles and open to challenging opportunities!
```
